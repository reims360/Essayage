...
const POSE_INSTRUCTIONS = [
  "Vue de face, mains sur les hanches",
  "Légèrement tourné, vue de 3/4",
  "Plan américain",
  "Vue de profil",
  "Sautant en l'air, prise en pleine action",
...
const App: React.FC = () => {
  ...
  const handleGarmentSelect = useCallback(async (garmentFile: File, garmentInfo: WardrobeItem) => {
    ...
    if (nextLayer && nextLayer.garment?.id === garmentInfo.id) {
        ...
        setCurrentPoseInstruction(POSE_INSTRUCTIONS[0]); // Reset pose when changing layer
        return;
    }

    setError(null);
    setIsLoading(true);
    setLoadingMessage(`Ajout de ${garmentInfo.name}...`);
    ...
    // FIX: Changed `err: unknown` to `err` to resolve a type conflict with the error handling utility.
    } catch (err: any) {
      // FIX: Pass the raw unknown error object to getFriendlyErrorMessage for proper handling.
      setError(getFriendlyErrorMessage(err, "Échec de l'application du vêtement"));
    } finally {
      setIsLoading(false);
      setLoadingMessage('');
    }
  }, [displayImageUrl, isLoading, currentPoseInstruction, outfitHistory, currentOutfitIndex]);

  const handleRemoveLastGarment = () => {
    if (currentOutfitIndex > 0) {
      setCurrentOutfitIndex(prevIndex => prevIndex - 1);
      setCurrentPoseInstruction(POSE_INSTRUCTIONS[0]); // Reset pose to default when removing a layer
    }
  };
  
  const handlePoseSelect = useCallback(async (newInstruction: string) => {
    if (isLoading || outfitHistory.length === 0 || newInstruction === currentPoseInstruction || !newInstruction.trim()) return;
    
    const currentLayer = outfitHistory[currentOutfitIndex];

    // If pose already exists, just update the instruction to show it.
    if (currentLayer.poseImages[newInstruction]) {
    ...
    setError(null);
    setIsLoading(true);
    setLoadingMessage('Changement de pose...');
    
    const prevInstruction = currentPoseInstruction;
    // Optimistically update the pose instruction so the pose name changes in the UI
    setCurrentPoseInstruction(newInstruction);
    ...
    // FIX: Changed `err: unknown` to `err` to resolve a type conflict with the error handling utility.
      ...
      // FIX: Pass the raw unknown error object to getFriendlyErrorMessage for proper handling.
      setError(getFriendlyErrorMessage(err, 'Échec du changement de pose'));
      // Revert pose instruction on failure
      setCurrentPoseInstruction(prevInstruction);
    } finally {
      setIsLoading(false);
      setLoadingMessage('');
    }
            ...
            <main className="flex-grow relative flex flex-col md:flex-row overflow-hidden">
              ...
              >
                  <button 
                    ...
                    aria-label={isSheetCollapsed ? 'Agrandir le panneau' : 'Réduire le panneau'}
                  >
